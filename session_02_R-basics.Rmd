...
```
## Introduction
This session is a quick introduction to R. It's written for those without prior programming experience, and it concentrates on demonstrating how the main building blocks of R code work. The aim of this session is to help you develop your skills in reading and writing R code. These building blocks are:
...
We will also introduce some of the technical terms used to describe code in R with the intention of making some of the useful online sources of help (such as [stack overflow](https://stackoverflow.com/questions/tagged/r)) more accessible and useful.
...
data_dates <- data %>% mutate(date = dmy(date))
...
```{r}
data3 <- read_csv("data/data3.csv") %>% mutate(date = dmy(date)) 
head(data3)
```
...
data3 %>% filter(if_any(c(date, count1, count2, cnont3), ~ is.na(.)))
