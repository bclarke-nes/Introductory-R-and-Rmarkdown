...
This resource is a brief introduction to writing dynamic reports using [R](https://www.r-project.org/), [Rmarkdown](http://rmarkdown.rstudio.com), and [RStudio Cloud](https://rstudio.cloud). It runs as a series of three interactive sessions, and is designed for those without previous programming experience working in health, care, and housing. It gives a general introduction to the tools, and then some useful examples of frequently-met data-driven tasks. It is intended as a starting point for automating your report writing processes, with the aim of replacing, enhancing, or simplifying, manual report writing. The demonstration also covers ways of producing the report in a variety of formats including .pdf, .docx (Word format), and .html (webpage).
...
Note too that it is a learning resource. You'll need some time to read this introduction and get set for the training session, as well as for the training sessions themselves. 
...
You'll need to do a little bit of preparation before the training session, which should take about 15 minutes to complete. Please do this before the start of the first session so that we can make a prompt start. If you're new to [RStudio Cloud](https://rstudio.cloud), please follow the step-by-step instructions below. If you've worked with RStudio Cloud before, you can just signing-in to your account at [RStudio Cloud](https://rstudio.cloud/), create a new project from the [GitHub Repository at https://github.com/bclarke-nes/Dynamic-report-demo](https://unito.io/blog/guide-to-github-for-project-managers/), and then open the `demo.Rmd` file from the file pane.
...
3. Once you've signed-in to RStudio Cloud, add a new project by clicking ```New Project >> New Project from a Git repository```. When prompted, enter the URL `https://github.com/bclarke-nes/Dynamic-report-demo`
...
Don't worry is this first sight of the the [RStudio Cloud](https://rstudio.cloud) interface is a shock. There's an awful lot happening here, and most of the apparent complexity is just information overload! We'll start adapting the interface to be more friendly at the start of the first interactive session.
