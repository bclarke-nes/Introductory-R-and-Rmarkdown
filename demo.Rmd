...
+ ```demo.csv``` is where we tell `read.csv` which .csv file to open by specifying the file's path
...
Here's the most important point in the whole of this demonstration: we can make our RMarkdown text change depending on the data that we feed to R. Let's start with a group of examples:

### Text showing data from `r month(min(demo_dates$date), label = T, abbr=F)` `r year(min(demo_dates$date))`

+ we can count up all of the rows here, and by using the expression `` `r '\x60r nrow(demo_dates)\x60'` ``
find out that we have a total of `r nrow(demo_dates)` rows in our tibble
+ using the command `sum(demo_dates$count1)` to add all the entries in the count1 column to give a total of `r sum(demo_dates$count1)`
+ `max(demo_dates$count3)` will let us discover that the maximum value in count3 is `r max(demo$count3)`. 
+ or we can say that on the first day of this range (`r min(demo_dates$date)`), the value of count2 was `r demo_dates[1,3]`
+ or that the mean (average) of count3 was `r round(mean(demo_dates$count3), 1)`
...
Note the slightly different mini-chunk structure using single backticks that we've employed here - with a general format of `` `r '\x60r command()\x60'` ``. This tells R/Rmarkdown to run the command, but pass the output to RMarkdown which displays the output as if it were written inline.

Now let's change the data, and run exactly the same list of queries again. There are a couple of R code chunks here that we don't need to show in this report - but do feel free to look at the code pane to see what's going on behind the scenes:
...
demo_dates <- read_csv("demo4.csv") %>% mutate(date=dmy(date))
```
### Text showing data from `r month(min(demo_dates$date), label = T, abbr=F)` `r year(min(demo_dates$date))`
...
<!-- Please ignore the horrible messing about with \x60r etc that I've done in the paragraph below to get backticks to display properly in RMarkdown. This isn't something that you're likely to have to do - unless you end up writing your own RMarkdown tutorial at some point in the future...-->
