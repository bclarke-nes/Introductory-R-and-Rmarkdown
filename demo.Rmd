...
There's a quick reference, and a couple of exercises, at the foot of this section. But to give you an idea of what Rmarkdown can do, we can have a look at the introductory materials for this demonstration. While this report is completely static - there's no data in it, and apart from the date at the top nothing changes without us manually updating it - it's still a useful demonstration of the kinds of formatting that we can do in Rmarkdown. Let's have a quick look at the code for the introduction now to see what it's like:

### Rmarkdown in this report

1. First, open the `intro.Rmd` file from the files pane by clicking on the name

```{r echo=F, out.width="300px", fig.align='center'}
knitr::include_graphics("img/load_intro.png")
```

2. This opens the file in the code pane. Note that we've now got two tabs at the top of the code pane - we can switch quickly between open files using these as you would in a web browser

```{r echo=F, out.width="300px", fig.align='center'}
knitr::include_graphics("img/tabs.png")
```

3. Once we've opened the `intro.Rmd` file, `knit` it to .html, like we did in the previous section. 

4. You can then follow along between the code and viewer panes. Ignoring the first 20 lines or so - which are used to set the display options for the report - you can see that most of the code is just the displayed text. See if you can spot:
...
5. You'll also see that we were able to output this report in different formats (the `.html` you've viewing now, versus the `.pdf` that you received before this session). Now might be a good time to try knitting to one of the other formats available - like `.doc`, or `.pdf`. Note that the formatting is (broadly) the same, no matter what the output format. This can be a fantastic time-saver if you're manually producing both print and web versions of your reports now.

6. When you've finished looking at the Rmarkdown, be sure to switch back to the `demo.Rmd` tab in the code window, and re-knit the demo report
...
# Write your code below. This will overwrite the contents of demo2 with new data. Again, the previous line of R code is a useful guide: but you'll need to change the command, and the name of the data file the command loads.
